$(function () {
    $.ajax({
        ...
        success: function (response) {
            ...
            const bytes = new Uint8Array(window.atob(response.content).split('').map((char) => char.charCodeAt()))
    ...
    $('.container').scroll(function () {
        ...
        if (state.highlightedPage === 2) {
            ...
            state.focusedEntry =
                Math.floor((scrollDist + entryHeight / 2 - 2 * em) / entryHeight) + 1
    ...
    function highlightPage(x, y) {
        ...
        if (!state.transitioning) {
            ...
            if (newPage !== 0) {
                ...
                body.css('cursor', state.mouseDown ? 'grabbing' : 'grab')
            }
        }
