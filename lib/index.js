$(function () {
    ...
    if ('maxTouchPoints' in navigator) {
        state.touchScreen = navigator.maxTouchPoints > 0
    } else if ('msMaxTouchPoints' in navigator) {
        state.touchScreen = navigator.msMaxTouchPoints > 0
    } else {
        const mQ = window.matchMedia && matchMedia('(pointer:coarse)')
        if (mQ && mQ.media === '(pointer:coarse)') {
            state.touchScreen = !!mQ.matches
        } else if ('orientation' in window) {
            state.touchScreen = true // deprecated, but good fallback
        } else {
    ...
    theWindow
        .resize(function () {
            console.log(`window: ${theWindow.height()} x ${theWindow.width()}`)
            console.log(`wall: ${$wall.height()} x ${$wall.width()}`)
    ...
    $('.container').scroll(function () {
        if (state.highlightedPage === 2) {
            ...
            state.focusedEntry =
                Math.floor(($('.container').scrollTop() + entryHeight / 2 - 2 * em) / entryHeight) + 1
